...
void addEmployee(string name_input, double salary_input, empType type)
{
	...
	if (type == 0)				 // If employee type is equal to 0, then create a nontechnical employee profile
	{
		newEmp = new Nontechnical(name_input, salary_input, type);
	}
	else						 // Otherwise, create technical employee profile
	{
		newEmp = new Technical(name_input, salary_input, type);
	}

	if (tempList = NULL)		 // If list is empty, then add to the beginning
	{
		tempList = new Container();
		tempList->emp = newEmp;
		list = tempList;
	}
	else                         // Otherwise if list has a beginning node/profile, add to the next position of the list
	{
		tempList = new Container();
		tempList->emp = newEmp;
		tempList->next = list;
...
{
	Container* tempList = list;			// Work on a copy of 'list'
	if (tempList == NULL)				// If list is empty, display the end of the list
	{
		cout << "\n" << endl;
		cout << "(end of list)" << endl;
		return;
	}
	else                                // Otherwise, display the filled in list
	{
		while (tempList != NULL)
		{
...
{
	Container* tempList = list;		// Work on a copy of 'list'
	std::ofstream textFile;			// File variable named 'textFile'	
	int count = 0;					// A local counter variable set to dummy value '0'

	while (tempList != NULL)		// While the list is not empty
	{
		tempList = tempList->next;
		count++;
	}
	...
	if (textFile.is_open())			// If the file is open, then write the input content in the file
	{
		tempList = list;
		textFile << count << endl;
		while (tempList != NULL)
		{
			textFile << tempList->emp->getName() << endl;	// Write down Employee Name
			textFile << tempList->emp->getSalary() << endl;	// Write down Employee Salary
			empType eType = tempList->emp->getEmpType();	// Write down Employee Type as either 'Nontechnical' or 'Technical' 
			if (eType == 0)
			{
				textFile << "Nontechnical" << endl;
			}
			else 
			{
				textFile << "Technical" << endl;
			}
			tempList = tempList->next;						// Next position
		}
...
{
	Container* tempList = list;		// Work on a copy of 'list'
	std::ifstream textFile;			// File variable named 'textFile'
	int count = 0, x;				// A local counter variable set to dummy value '0' along with variable x for the 'for' loop
	
	textFile.open(fileName);		// Open the file		
							
	if (textFile.is_open())			// If the file is open, then load the contents from the file to the global linked list
	{
		textFile >> count;

		for (x = 0; x < count; x++) // Recapitulate throughout the list
		{
			Container* newList = new Container(); // Create new Container
			string empName;						  // String named 'empName'
			int empSal;							  // Integer named 'empSal'
			string eType;						  // String named 'eType'

			getline(textFile, empName); // Fix extra line
			getline(textFile, empName);

			textFile >> empSal;			// 'textFile' set to 'empSal'
			textFile >> eType;			// 'textFile' set to 'eType'

			empType employeeType;		// empType named 'employeeType'

			if (eType == "nontechnical")
			{
			...
			addEmployee(empName, empSal, employeeType); // Add content to the global linked list
		}
	}
	textFile.close();			// Close the file
	return;	
